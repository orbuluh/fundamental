...
- You are also given two integers k and x. We want to hire exactly k workers according to the following rules:
- You will run k sessions and hire exactly one worker in each session.
- In each hiring session, choose the worker with the lowest cost from either the first x workers or the last x workers. Break the tie by the smallest index.

- For example, if costs = [3,2,7,7,1,2] and x = 2, then in the first hiring session, we will choose the 4th worker because they have the lowest cost [{3,2},7,7,{1,2}].
